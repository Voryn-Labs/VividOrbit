...
class WalkthroughController(
    ...
    private fun lightChip(keyCode: Int) {
        val id = when (keyCode) {
            ...
            KeyEvent.KEYCODE_LAST_CHANNEL -> 3
            else -> return
        }
        litChips.add(id)
        bindChips()
    ...
    private fun onDemoActivated(index: Int) {
        val channel = channels[index]
        if (currentPage == 2) {
            val result = assignDemoNumberOne(channels, channel.id) ?: return
            ...
            statusView.text = if (result.swappedWith != null) {
                ...
                overlay.context.getString(R.string.walkthrough_status_assigned, channel.name)
            }
            bindDemoRows()
